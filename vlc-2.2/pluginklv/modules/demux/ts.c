...
static void ParsePES( demux_t *p_demux, ts_pid_t *pid, block_t *p_pes )
{
    ...
    if(i_pts > VLC_TS_INVALID && pid->es->fmt.i_cat == VIDEO_ES)
    {
        msg_Dbg(p_demux, "MAGNAROX : Save last video pts");
        p_sys->last_pts = i_pts;
    }


    if(i_pts <= VLC_TS_INVALID && pid->es->fmt.i_codec == VLC_FOURCC( 'k', 'l', 'v', ' ' ))
    {
        msg_Dbg(p_demux, "MAGNAROX : Apply video pts on KLV");
        i_pts = p_sys->last_pts;
    }
...
static void PIDFillFormat( es_format_t *fmt, int i_stream_type )
{
    ...
    case 0x06:  /* PES_PRIVATE  (fixed later) - MAGNAROX -> Force Audio*/
        es_format_Init( fmt, AUDIO_ES, VLC_FOURCC( 'k', 'l', 'v', ' ' )  );
        break;
...
static void PMTCallBack( void *data, dvbpsi_pmt_t *p_pmt )
{
    ...
    for( p_es = p_pmt->p_first_es; p_es != NULL; p_es = p_es->p_next )
    {
        ...
        msg_Dbg(p_demux, "MAGNAROX : pid=%d type=%d", p_es->i_pid, p_es->i_type);
        ...
        if ( p_es->i_type == 0x06 )
        {
            msg_Dbg( p_demux, "MAGNAROX : Force Private Stream In AUDIO KLV");
        }
