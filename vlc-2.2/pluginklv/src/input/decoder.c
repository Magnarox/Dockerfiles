...
{
    msg_Dbg(p_dec, "MAGNAROX : Try to play audio");
    decoder_owner_sys_t *p_owner = p_dec->p_owner;
...
{
    msg_Dbg(p_dec, "MAGNAROX : Try to decode audio");
    decoder_owner_sys_t *p_owner = p_dec->p_owner;
    block_t *p_aout_buf;
    int i_decoded = 0;
    int i_lost = 0;
    ...
    else while( (p_aout_buf = p_dec->pf_decode_audio( p_dec, &p_block )) )
    {
        ...
        {
            msg_Dbg(p_dec, "MAGNAROX : bad PTS - me : %d - Owner : %d", p_aout_buf->i_pts , p_owner->i_preroll_end );
            block_Release( p_aout_buf );
            continue;
        }
